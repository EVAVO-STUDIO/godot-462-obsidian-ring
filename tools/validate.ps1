...
$Required = @(
    ...
    'scripts/match_rules.gd','scripts/team_play_rules.gd','data/teams.json','data/rules.json',
    'data/courts.json','data/league.json','docs/GAME_DESIGN.md','docs/ARCHITECTURE.md','docs/QA.md'
)
foreach ($RelativePath in $Required) {
    if (-not (Test-Path (Join-Path $Root $RelativePath))) { throw "Missing required file: $RelativePath" }
}

foreach ($JsonPath in @('data/teams.json','data/rules.json','data/courts.json','data/league.json')) {
...
if (-not $Godot) {
    Write-Warning 'Godot executable not found. Structural, catalogue and team-play validation passed; engine smoke test skipped.'
    exit 0
}
