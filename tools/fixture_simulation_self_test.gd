...
func _test_simulation() -> void:
	...
	var table := [{"id":"obsidian_guard","played":0},{"id":"sun_serpents","played":0}]
...
func _test_schedule_balance() -> void:
	...
	var homes: Dictionary = {}; var games: Dictionary = {}; var pair_counts: Dictionary = {}
	for round_data in rounds:
		var seen: Dictionary = {}
		for fixture in round_data.get("fixtures", []):
			var home := str(fixture.get("home", "")); var away := str(fixture.get("away", ""))
			...
			seen[home] = true; seen[away] = true
			homes[home] = int(homes.get(home, 0)) + 1; games[home] = int(games.get(home, 0)) + 1; games[away] = int(games.get(away, 0)) + 1
			var pair := [home, away]; pair.sort(); var key := "%s|%s" % [pair[0], pair[1]]; pair_counts[key] = int(pair_counts.get(key, 0)) + 1
	...
	for count in pair_counts.values(): pair_values.append(int(count))
	pair_values.sort(); _expect(pair_values == [3,3,3,3,4,4], "ten-round opponent frequencies should be optimally balanced")
...
func _test_result_round_lifecycle() -> void:
	...
	if main_file != null:
		var source := main_file.get_as_text(); var result_branch := source.find("GamePhase.RESULT:"); var advance_call := source.find("_advance_between_matches()", result_branch); var increment := source.find("match_number += 1", result_branch)
...
func _test_ai_roster_recovery_boundary() -> void:
	var fixture_file := FileAccess.open("res://scripts/fixture_simulation_director.gd", FileAccess.READ)
	_expect(fixture_file != null, "fixture simulation director should be readable for AI recovery checks")
	if fixture_file == null: return
	var source := fixture_file.get_as_text()
	_expect(source.contains("var _last_match_number := -1"), "AI recovery should track the last processed round")
