...
func _reset_positions() -> void:
	...
	for i in range(home_players.size()):
		var p: Dictionary = home_players[i]; p["position"] = home_spawns[i]; home_players[i] = p
...
func _update_controlled_player(delta: float) -> void:
	...
	if possession_team == 1 and possession_index == controlled_home_index:
		...
		ball_position = carrier + Vector2(16,3)
...
func _update_team_ai(players: Array, home: bool, delta: float) -> void:
	for i in range(players.size()):
		...
		if owns_ball:
			target = Vector2(COURT.end.x - 70, COURT.get_center().y) if home else Vector2(COURT.position.x + 70, COURT.get_center().y)
		elif possession_team == 0 and TeamPlayRules.nearest_player_index(players, ball_position) == i:
...
func _resolve_tackles() -> void:
	...
	if possession_team == 1 and possession_index >= 0:
		var carrier: Vector2 = home_players[possession_index]["position"]
...
func _clamp_to_court(point: Vector2) -> Vector2:
	return Vector2(clampf(point.x, COURT.position.x + 12, COURT.end.x - 12), clampf(point.y, COURT.position.y + 12, COURT.end.y - 12))
...
func _configure_input() -> void:
	...
	_add_key_action("pass_ball",KEY_SPACE); _add_key_action("strike_ball",KEY_X); _add_key_action("tackle",KEY_Z); _add_key_action("switch_player",KEY_C)
	_add_key_action("confirm",KEY_ENTER); _add_key_action("cancel",KEY_ESCAPE); _add_key_action("restart",KEY_R)
